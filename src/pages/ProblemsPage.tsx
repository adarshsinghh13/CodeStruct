...
const ProblemsPage = () => {
  ...
  // Ensure problems are fetched immediately
  useEffect(() => {
  ...
  const fetchProblems = async () => {
    ...
    try {
      // If user is not logged in, only show hardcoded problems
      if (!user) {
        console.log("User not logged in - showing sample problems only");
        const hardcodedProblems = getHardcodedProblems();
        setProblems(hardcodedProblems);
        setFilteredProblems(hardcodedProblems);
        setLoading(false);
        return hardcodedProblems;
      }
      
      console.log("User logged in - fetching all problems from database");
      // Optimize the query to only select the fields we need
      ...
      if (error) {
        ...
        toast.error("Error loading problems: " + error.message);
        
        // Fall back to hardcoded problems if database fails
      ...
      if (!data || data.length === 0) {
        console.log("No problems returned from database - using hardcoded problems");
        
        // Fall back to hardcoded problems if no data returned
        const fallbackProblems = getHardcodedProblems();
        setProblems(fallbackProblems);
        setFilteredProblems(fallbackProblems);
      ...
      console.log("Problems found:", problemsData.length);
      ...
      // Fall back to hardcoded problems on error
      const fallbackProblems = getHardcodedProblems();
      setProblems(fallbackProblems);
      setFilteredProblems(fallbackProblems);
  ...
  if (isAdminMode && isAdmin) {
    return (
      <div className="container mx-auto px-4 py-8">
        ...
        <AdminProblemManager onProblemsUpdate={fetchProblems} />
      </div>
    );
  }
