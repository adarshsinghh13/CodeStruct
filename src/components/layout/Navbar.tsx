...
import { Sun, Moon, User, LogOut, MessageSquare, Shield } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
const Navbar = () => {
  ...
  const isAdmin = user?.email === 'adarshsinghh13@gmail.com';
      ...
      <div className="container flex h-16 items-center justify-between px-4 md:px-6">
        ...
        <div className="hidden md:flex items-center gap-6">
          ...
          {isAdmin && (
            <Link 
              to="/messages" 
              className={`transition-colors flex items-center gap-1 ${isActive('/messages') 
                ? 'text-foreground font-medium' 
                : 'text-foreground/80 hover:text-foreground'}`}
            >
